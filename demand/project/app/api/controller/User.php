...
use app\common\enum\NoticeState;
...
//        $update_data['city'] = $data['city'];
    ...
    public function notice_list(){
        ...
        if(!empty($bottom_id))$notice_where['noc.notice_id'] = array('lt',$bottom_id);
        $notice_list= IDb::getInstance('notice as noc')
            ->setDbFiled('noc.*,dd.demand_describe as notice_title,dd.demand_status,noc.demand_id')
            ->setDbJoin('demand as dd','dd.demand_id = noc.demand_id')
            ->setDbOrder("noc.notice_id desc")
            ->setDbLimit(10)
        ...
        foreach ($notice_list as $key=>$value){
            $notice_list[$key]['demand_status'] = HtmlEnumValue::$enum_value['demand_status'][$value['demand_status']];
        }
        //返回数据
        self::set_code(self::SUCCESS);
    ...
    public function notice_read(){
        ...
        $where['user_id'] = self::$uid;
        $where['notice_read'] = IsRead::No;
        $notice_count = IDb::getInstance('notice')->setDbWhere($where)->con();
        if ($notice_count === false){
            self::set_code(self::WARNING_0002);
            self::set_msg('获取未读消息失败');
            self::send();
        }
        //返回数据
        self::set_code(self::SUCCESS);
        self::set_value('notice_count', $notice_count);
        return self::send();
    }
    /**
     * @功能：修改用户信息
