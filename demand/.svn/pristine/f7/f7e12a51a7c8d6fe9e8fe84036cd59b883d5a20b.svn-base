...
class Demand extends DemandBaseController{
    ...
    public function add_demand(){
        ...
        $user_info = IDb::getInstance('user')->setDbFiled('*,if(real_name is null,nickname,real_name) as real_name')->setDbWhere($where)->row();
        //项目信息
        ...
        $sendText = $user_info['real_name'].'，提交了新的需求请审核；（项目编号：'.$project_replenish.'，需求编号：'.$demand_replenish.'）';
    ...
    public function get_demand_list(){
        ...
        if (!empty($search_content)){
            if (is_numeric($search_content)) {
                $demand_where['dd.demand_id'] = $search_content + 0;
            } else {
                $demand_where['concat(ur.real_name,dd.demand_describe)'] = array("like","%{$search_content}%");
            }
        }
        if(!empty($demand_status))$demand_where['dd.demand_status'] = $demand_status;
        if(!empty($bottom_id))$demand_where['dd.demand_id'] = array('lt',$bottom_id);
        //取得数据
        ...
        foreach ($demand_list as $key=>$demand) {
            $demand_number = str_pad($demand['demand_id'],6,"0",STR_PAD_LEFT);

            $demand_list[$key]['demand_number'] = $demand_number;
            $test_mans = array();
            if ($demand['test_list']) {
    ...
    public function get_task(){
        ...
        $design_task_list = IDb::getInstance('task as tk')
            ...
            ur.phone,ur.avatar,substring(tk.create_time,1,16) as create_time,
            substring(tk.end_time,1,16) as end_time,substring(tk.start_time,1,16) as start_time"])
