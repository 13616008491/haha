...
class CheckBox extends Controller{
    ...
    /**
     * @功能：单选框设置
     * {:W('Common/Checkbox/checkbox',array("input_name","sex",$value))}
     * {:W('Common/Checkbox/checkbox',array("input_name",array($data,"value","name"),$value))}
     ...
     * @return mixed
     */
    public static function checkbox($input_name,$data,$default=null,$class=null,$style=null,$required=false,$disabled=false) {
        //判断参数是否正确
        ...
        if(is_string($data)){
            ...
            foreach($data as $key=>$val){
                //判断默认值
                if(!empty($default) && in_array($key,$default)){
                    array_push($html,"<label {$property}><input name='{$input_name}[]' type='checkbox' value='{$key}' checked='checked' {$disabled} {$validate}/>{$val}</label>");
                }else{
                    array_push($html,"<label {$property}><input name='{$input_name}[]' type='checkbox' value='{$key}' {$disabled} {$validate}/>{$val}</label>");
                }
            }
        }else{
            //判断值是否为空；
            if(is_array($data)){
                ...
                foreach($data_list as $item){
                    //判断默认值
                    if(!empty($default) && in_array($item[$key],$default)){
                        array_push($html,"<label {$property}><input name='{$input_name}[]' type='checkbox' value='{$item[$key]}' checked='checked' {$disabled} {$validate}/>{$item[$val]}</label>");
                    }else{
                        array_push($html,"<label {$property}><input name='{$input_name}[]' type='checkbox' value='{$item[$key]}' {$disabled} {$validate}/>{$item[$val]}</label>");
                    }
                }
            }
        }
