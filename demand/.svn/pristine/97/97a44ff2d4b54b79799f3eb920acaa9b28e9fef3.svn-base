...
use app\agent\cache\IAgentAuthorityCache;
...
class AgentBaseController extends CommonController {
    ...
    public function _initialize(){
        ...
        $agent_role_id = get_login_agent_id();
        $agent_super = get_login_agent_super();
        ...
        if(!in_array($current_controller,$auth_controller)){
            ...
            //判断是否为超级管理员
            if($agent_super == AdminSuper::Ordinary){
                //缓存角色的功能数据权限数据
                $role_model = IAgentAuthorityCache::getAuthorityModel($agent_role_id);
        ...
        $role_menu = IAgentAuthorityCache::getAuthorityMenu($menu_model, $agent_role_id);
