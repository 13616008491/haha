...
class Total extends AdminBaseController {
    ...
    public function weekly_list(){
        ...
        $date_range_start = IRequest::get('date_range_start');
        $date_range_end = IRequest::get('date_range_end');
        $is_history = IRequest::get('is_history');

        //设置查询条件
        $weekly_where = array();
        if(!empty($weekly_id)) $weekly_where['weekly_id'] = $weekly_id;
        if(!empty($date_range_start)) $weekly_where['date_range_start'] = $date_range_start;
        if(!empty($date_range_end)) $weekly_where['date_range_end'] = $date_range_end;
        ...
        $this->assign('date_range_start', $date_range_start);
        $this->assign('date_range_end', $date_range_end);
     ...
     * @开发者：wdd
     */
    public function weekly_history(){
        //接收参数
        $weekly_id = IRequest::get("weekly_id",IRequest::NOT_EMPTY,"周报编号不能为空！");

        $where['weekly_id'] = $weekly_id;
        $data['is_save'] = IsDelete::Yes;
        $weekly_upd = IDb::getInstance('weekly')
            ->setDbData($data)
            ->setDbWhere(array('weekly_id'=>$weekly_id))
            ->upd();
        if(empty($weekly_upd)){
            error("存档失败！");
        }
